...
addEventSubmit.addEventListener("click", () => {
  ...
  if (
    ...
    alert("Formato de tempo Invalido!");
    return;
  }
...
/*----------------------------------------------------*/
let loginAttempted = false; // Rastreia se o login foi tentado

document.querySelector('.add-event').addEventListener('click', function() {
    if (!loginAttempted) {
        document.getElementById('admin-login-modal').style.display = 'block'; // Exibe o modal de login
    } else {
        document.querySelector('.add-event-wrapper').style.display = 'block'; // Exibe a seção de adicionar evento
    }
});
...
document.getElementById('admin-login-btn').onclick = function() {
    ...
    if (username === 'adm' && password === '123') {
        document.getElementById('admin-login-modal').style.display = 'none'; // Fecha o modal
        document.querySelector('.add-event-wrapper').style.display = 'block'; // Exibe a seção de adicionar evento
        document.getElementById('login-error').style.display = 'none'; // Esconde mensagem de erro
        loginAttempted = true; // Marca que o login foi bem-sucedido
    } else {
...
window.onclick = function(event) {
    if (event.target == document.getElementById('admin-login-modal')) {
        document.getElementById('admin-login-modal').style.display = 'none'; // Fecha o modal
        loginAttempted = false; // Permite reabertura do modal na próxima tentativa
    }
};